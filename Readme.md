...
graph TD
    USB[USB-C Connector] -->|5V VBUS| Charger[Battery Charger / PMIC]
    Charger -->|VBAT| Batt[3.7V Li-Po Battery]
    Batt -->|VBAT| DCDC[3.3V DC/DC Regulator]
    
    DCDC -->|3V3 VCC| MCU[nRF52840 SoC]
    DCDC -->|3V3 VCC| IMU[BMA253 Accelerometer]
    DCDC -->|3V3 VCC| Disp[E-Paper Display]
    
    MCU <-->|SPI| Disp
    MCU <-->|I2C| IMU
    MCU -->|PWM| Motor[Vibration Shaker Motor]
    
    Btn[3x Navigation Buttons] -->|GPIO| MCU
    MCU ---|RF Trace| Ant[2450AT 2.4GHz Antenna]
...
I2C SDA	P0.02	Data	Shared I2C bus for the IMU sensor
I2C SCL	P0.03	Clock	Shared I2C bus for the IMU sensor
SPI SCK	P0.31	Clock	Dedicated high-speed SPI for E-Paper Display
SPI MOSI	P0.29	Data Out	Dedicated high-speed SPI for E-Paper Display
SPI CS	P0.30	Chip Sel	Dedicated high-speed SPI for E-Paper Display
USB D+ / D-	D+ / D-	USB Data	Differential pair for USB communication
ANT1	ANT	RF Out	50-ohm matched trace to 2.4GHz chip antenna
Button Up	P0.24	Input	Navigation switch (Active Low with internal pull-up)
Button Ent	P1.00	Input	Enter / Confirm switch (Active Low)
Button Dn	P0.22	Input	Navigation switch (Active Low)
Vibe Motor	P0.16	Enable	PWM output to drive the shaker motor MOSFET
Design Log
...
During the layout and routing phases of the PCB, several specific design rules and constraints were strictly applied to ensure manufacturability and signal integrity:
    ...
    2-Layer Grounding Strategy: Because routing was done on both the Top and Bottom layers, solid Ground (GND) polygons were poured on both layers. Extensive via stitching was applied across the board to connect these planes, reducing the return path impedance and minimizing EMI.
    ...
    No Right Angles: All traces were routed using 45-degree angles to prevent acid traps during the etching process and to reduce signal reflections on high-speed lines.

    DRC Exceptions & Mechanical Fit: Dimension and overlap DRC errors related to the three tactile buttons and the USB-C connector were intentionally accepted. These components overhang the PCB edge slightly to perfectly align with the mechanical cutouts of the provided 3D enclosure.
